...
# DIRETRIZ: Fusão de Código Limpa. Preservação estrita da UI do "Código 1"
#           com injeção da Lógica de Dados (Schema/Destruição/Zebra) do "Código 2".
# =====================================================================
...
# Cores Injetadas do Código 2 para Zebra Striping
COR_ZEBRA_1 = "#1A1A28"     
COR_ZEBRA_2 = "#1E1E2C"
...
class PlataformaFinanceira(ctk.CTk):
    ...
    def inicializar_banco_relacional(self, db_path, chave_hex):
        """[INJEÇÃO DO CÓDIGO 2] Destruição de Schema Antigo e Nova DDL"""
    ...
    def tentar_login(self):
        ...
        try:
            ...
            # [INJEÇÃO CÓDIGO 2] Auto-healing (Força criação do novo schema em base legada)
            try:
    ...
    def rotear_tela(self, tela_destino, kwargs=None):
        ...
        # -------------------------------------------------------------
        # ROTA: PLANEJAMENTO [INJEÇÃO DO CÓDIGO 2 MANTENDO UI CÓDIGO 1]
        # -------------------------------------------------------------
            ...
            # --- ESTADO 1: ORÇAMENTOS (LISTA ZEBRA) ---
            if aba == 'orcamentos':
                ...
                def gravar_despesa():
                    if e_nome_desp.get() and e_preco.get():
                        try:
                            ...
                            c.execute("INSERT INTO tb_orcamentos (nome, tipo, data_vencimento, valor, mes_referencia) VALUES (?, ?, ?, ?, ?)", 
                                      (e_nome_desp.get(), cb_tipo_desp.get(), d_iso, abs(float(e_preco.get().replace(',','.'))), mes))
                            self.db_conn.commit()
            ...
            # --- ESTADO 2: METAS DE CAPITAL ---
                ...
                def gravar_meta():
                    try:
                        ...
                        t_alvo = e_alv.get()
                        t_ap = e_ap.get()
                        if not n_meta or not t_alvo: return
