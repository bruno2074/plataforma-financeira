...
# DIRETRIZ: Resolução Silenciosa. Correção do Módulo de Orçamentos (Tratamento
#           de Exceções de Data) e Injeção do Console SysAdmin (Raw DB).
# =====================================================================
...
COR_ZEBRA_1 = "#1A1A28"     # Zebra striping (Ímpar)
COR_ZEBRA_2 = "#1E1E2C"     # Zebra striping (Par)
COR_DESTAQUE = "#B026FF"    # Roxo Neon
...
class PlataformaFinanceira(ctk.CTk):
    ...
    def tentar_login(self):
        ...
        if self.modo_lockdown:
            if validar_chave_emergencia(usuario, self.entry_senha.get()):
        ...
        try:
            ...
            try:
                cursor.execute("CREATE TABLE IF NOT EXISTS tb_orcamentos (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT NOT NULL, tipo TEXT NOT NULL, data_vencimento DATE NOT NULL, valor REAL NOT NULL, mes_referencia TEXT NOT NULL)")
    ...
    def carregar_dashboard(self):
        ...
        # [SYSADMIN] Injeção de Rota Condicional para Admin
        if self.usuario_atual.lower() == 'admin':
            btn_sys = ctk.CTkButton(self.sidebar_frame, text="🛠️ Console SysAdmin", command=lambda: self.rotear_tela("sysadmin"), fg_color="transparent", hover_color=COR_DESPESA, anchor="w", font=ctk.CTkFont(size=14, weight="bold"), height=45, text_color=COR_DESPESA)
    ...
    def rotear_tela(self, tela_destino, kwargs=None):
        ...
        # -------------------------------------------------------------
        # [SYSADMIN] ROTA EXCLUSIVA DE ADMINISTRAÇÃO DDL/DML
        # -------------------------------------------------------------
        elif tela_destino == "sysadmin":
            if self.usuario_atual.lower() != 'admin': return # Constraint de Segurança
            ...
            entry_sql = ctk.CTkEntry(frame_query, placeholder_text="SELECT * FROM tb_transacoes LIMIT 10;", height=45, font=ctk.CTkFont(family="Courier", size=14))
            ...
            def run_query():
                ...
                try:
                    ...
                    c.execute(q)
                    # Verifica instrução para retorno de massa visual
                    if q.strip().upper().startswith("SELECT") or q.strip().upper().startswith("PRAGMA"):
                        ...
                        lbl_err.configure(text=f"Query executada com sucesso. Linhas devolvidas: {len(rows)}", text_color=COR_RECEITA)
                    else:
                        self.db_conn.commit()
                        lbl_err.configure(text="Comando DML/DDL executado com sucesso.", text_color=COR_RECEITA)
                except Exception as e:
                    lbl_err.configure(text=f"Erro SQL: {e}", text_color=COR_DESPESA)
                    
            ctk.CTkButton(frame_query, text="Executar", fg_color=COR_DESPESA, hover_color="#C0392B", height=45, font=ctk.CTkFont(weight="bold"), command=run_query).pack(side="right", padx=(0, 20), pady=15)

        # -------------------------------------------------------------
        # ROTA: LANÇAMENTOS (I/O)
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # ROTA: PLANEJAMENTO (COM TRATAMENTO DE EXCEÇÃO EM ORÇAMENTOS)
        # -------------------------------------------------------------
            ...
            if aba == 'orcamentos':
                ...
                # [ENG] Rótulo de Feedback para isolamento de exceções e alerta ao usuário
                lbl_fb_orc = ctk.CTkLabel(f_forjar, text="")
                lbl_fb_orc.pack()

                def gravar_despesa():
                    try:
                        if not e_nome_desp.get() or not e_preco.get():
                            raise ValueError("Nome e Preço são obrigatórios.")
                        if len(e_venc.get()) != 10:
                            raise ValueError("A Data deve seguir o padrão DD/MM/YYYY.")
                            
                        # Tratamento seguro contra datas vazias ou irracionais
                    ...
                    except Exception as err_orc: 
                        lbl_fb_orc.configure(text=f"Erro: {err_orc}", text_color=COR_DESPESA)

                ctk.CTkButton(f_forjar, text="Gravar Despesa", fg_color=COR_PRIMARIA, height=45, corner_radius=10, command=gravar_despesa).pack(pady=20, padx=20, fill="x")
                ...
                try:
                    ...
                    if not linhas:
                        ctk.CTkLabel(f_lista, text="Lista de orçamento vazia.", text_color=COR_TEXTO).pack(pady=20)
                    else:
                ...
                except Exception as e: ctk.CTkLabel(f_lista, text=f"Erro: {e}", text_color=COR_DESPESA).pack()
                ...
                e_nm = ctk.CTkEntry(f_meta, placeholder_text="Nome (Ex: Fundo Casa)", height=45, corner_radius=10)
                ...
                lbl_fb_meta = ctk.CTkLabel(f_meta, text="")
                lbl_fb_meta.pack()
                ...
                def gravar_meta():
                    try:
                        ...
                        t_alvo = e_alv.get()
                        t_ap = e_ap.get()
                        if not n_meta or not t_alvo: 
                            raise ValueError("Nome e Alvo são obrigatórios.")
                        v_alvo = float(t_alvo.replace(',','.'))
                        v_aporte = float(t_ap.replace(',','.')) if t_ap.strip() else 0.0
                        self.db_conn.cursor().execute("INSERT INTO tb_metas (nome, valor_alvo, valor_acumulado) VALUES (?, ?, ?)", (n_meta, v_alvo, v_aporte))
                        self.db_conn.commit()
                        self.rotear_tela("planejamento", {'aba': 'metas'})
                    except Exception as e: lbl_fb_meta.configure(text=f"Erro: {e}", text_color=COR_DESPESA)

                ctk.CTkButton(f_meta, text="Gravar Meta", fg_color=COR_PRIMARIA, height=45, corner_radius=10, command=gravar_meta).pack(pady=20, padx=20, fill="x")
