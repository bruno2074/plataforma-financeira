...
# DIRETRIZ: Zero Verborragia Externa. Explicações embutidas no código.
# 
# CORREÇÕES APLICADAS NESTA ITERAÇÃO:
# 1. Design System iOS/HUD: Paleta ajustada para fundos escuros e cards 
#    arredondados com acentos neon.
# 2. Universal Privacy Wrapper: Ocultação garantida em O(1) para TODAS as abas.
# 3. Micro-interações (Hover Lift): Event binding injetado para simular
#    elevação 3D e borda amarelo/verde ao passar o mouse.
# 4. Routing Aninhado: "Planejamento Financeiro" dividido em abas reais.
# 5. Barras de Progresso: Configuradas estritamente com cinza escuro (vazio)
#    e verde neon (preenchido).
# 6. Restauração do I/O: Módulo de Lançamentos reconstruído com segurança.
# =====================================================================
...
# --- PALETA DE CORES (Mix iOS Dark + Sci-Fi HUD) ---
COR_FUNDO = "#12121A"       # Fundo profundo (iOS Dark)
COR_CARD = "#1E1E2C"        # Cards elevados
COR_BORDAS = "#2A2A3D"      # Bordas neutras
COR_TEXTO = "#A0A0B5"       # Texto secundário
COR_TEXTO_FORTE = "#FFFFFF" # Texto primário
COR_PRIMARIA = "#7B61FF"    # Roxo Apple
COR_RECEITA = "#00FF9D"     # Verde Neon (Preenchimento de barras/Receitas)
COR_DESPESA = "#FF4757"     # Vermelho Coral (Despesas)
COR_HOVER_LIFT = "#29293D"  # Fundo sutilmente mais claro para o "Lift"
COR_HOVER_BORDA = "#F1C40F" # Amarelo sinalizador para hover

ctk.set_appearance_mode("dark")
ctk.set_default_color_theme("dark-blue")

# =====================================================================
# MÓDULO 1: MOTOR CRIPTOGRÁFICO E MFA DE HARDWARE
# Objetivo: Derivação de chaves via PBKDF2 atrelada ao MAC Address.
# =====================================================================
...
def validar_chave_emergencia(usuario, input_chave):
    shadow_file = f"shadow_{usuario.lower()}.json"
    if not os.path.exists(shadow_file): return False
...
class PlataformaFinanceira(ctk.CTk):
    def __init__(self):
        ...
        # Tuning da UI Nativa (Treeview) para combinar com o CustomTkinter
    ...
    # =====================================================================
    # MÓDULO 2: UTILITÁRIOS DE ALTA PERFORMANCE (UI & DADOS)
    # =====================================================================
    def aplicar_mascara_financeira(self, valor_numerico):
        """
        [ENG] Wrapper Universal de Privacidade. 
        Garante que a diretriz "Ocultar Valores" funcione em O(1) em todo o software.
        """
        if self.modo_privacidade: return "R$ ****"
        return f"R$ {valor_numerico:,.2f}"

    def aplicar_efeito_hover(self, widget):
        """
        [ENG] Micro-interação: Efeito 'Lift'.
        Altera o background para simular elevação e a borda para amarelo vibrante no mouse-over.
        """
        cor_padrao = widget.cget("fg_color")
        borda_padrao = widget.cget("border_color")

        def on_enter(e):
            widget.configure(fg_color=COR_HOVER_LIFT, border_color=COR_HOVER_BORDA, border_width=2)
        def on_leave(e):
            widget.configure(fg_color=cor_padrao, border_color=borda_padrao, border_width=1)
            
        widget.bind("<Enter>", on_enter)
        widget.bind("<Leave>", on_leave)
        # Propaga para os filhos para evitar interrupção do evento
        for child in widget.winfo_children():
            child.bind("<Enter>", on_enter)
            child.bind("<Leave>", on_leave)

    def ligar_clique_recursivo(self, widget, comando):
        """Garante que clicar em textos dentro de um card acione a função principal do card."""
    ...
    def inicializar_banco_relacional(self, db_path, chave_hex):
        """DDL: Estrutura isolada de Tenant (Multi-banco)."""
        ...
        cursor.executescript('''
            ...
            CREATE TABLE IF NOT EXISTS tb_orcamentos (id INTEGER PRIMARY KEY AUTOINCREMENT, categoria_id INTEGER NOT NULL, teto_gasto REAL NOT NULL, mes_referencia TEXT NOT NULL, FOREIGN KEY (categoria_id) REFERENCES tb_categorias(id) ON DELETE CASCADE);
    ...
    def construir_tela_login(self):
        ...
        # [ENG] Acordeão da Chave de Emergência solicitado
        ...
        # Campo Read-only para permitir cópia
    ...
    def tentar_login(self):
        ...
        try:
            ...
            # Auto-healing silencioso
            cursor.execute('''CREATE TABLE IF NOT EXISTS tb_orcamentos (id INTEGER PRIMARY KEY AUTOINCREMENT, categoria_id INTEGER NOT NULL, teto_gasto REAL NOT NULL, mes_referencia TEXT NOT NULL, FOREIGN KEY (categoria_id) REFERENCES tb_categorias(id) ON DELETE CASCADE)''')
    ...
    def carregar_dashboard(self):
        ...
        btn_nav("🎯 Planejamento Financeiro", lambda: self.rotear_tela("planejamento")).grid(row=3, column=0, padx=15, pady=5, sticky="ew")
        ...
        self.btn_privacidade = ctk.CTkButton(self.sidebar_frame, text="🕶 Ocultar Valores", fg_color=COR_BORDAS, hover_color=COR_FUNDO, command=toggle_privacidade, height=45)
    ...
    def rotear_tela(self, tela_destino, kwargs=None):
        ...
        if tela_destino == "dash":
            ...
            # --- CARD 1: CONTAS (Com Efeito Hover Levantador) ---
            ...
            self.aplicar_efeito_hover(card_contas) # <--- [ENG] Aplicação do Design 140%

            # --- CARD 2: MÉTRICAS GLOBAIS (Hover -> Histórico) ---
            ...
            self.aplicar_efeito_hover(card_extrato) # <--- [ENG] Aplicação do Design 140%
            ...
            if not self.modo_privacidade:
                try:
                    ...
                    if dados_bd:
                        ...
                        # Ajuste de cores para bater com o Design iOS/Dark
        ...
        # -------------------------------------------------------------
        # ROTA: GESTÃO E INSPEÇÃO ISOLADA
        # -------------------------------------------------------------
            ...
            ctk.CTkLabel(frame_lista, text="Instituições Ativas (Duplo Clique para Isolar)", font=ctk.CTkFont(weight="bold")).pack(pady=10)
            ...
            # [ENG] Correção do Bug de Telemetria: O evento extrai os dados e joga para a rota
            def drill_down(e):
                item = tree.selection()
                if item:
            ...
            try:
                cursor = self.db_conn.cursor()
                cursor.execute("""
                    SELECT c.id, c.nome, COALESCE(SUM(CASE WHEN cat.tipo_fluxo = 'RECEITA' THEN t.valor ELSE -t.valor END), 0)
                    FROM tb_contas c LEFT JOIN tb_transacoes t ON c.id = t.conta_id LEFT JOIN tb_categorias cat ON t.categoria_id = cat.id GROUP BY c.id
                """)
            ...
            entry_nome = ctk.CTkEntry(frame_cad, width=220, placeholder_text="Instituição (Ex: Itaú)", height=45, corner_radius=10)
            ...
            def salvar():
                if entry_nome.get():
                    self.db_conn.cursor().execute("INSERT INTO tb_contas (nome, tipo) VALUES (?, ?)", (entry_nome.get(), combo_tipo.get()))
                    self.db_conn.commit(); self.rotear_tela("detalhes_contas")
        ...
        # -------------------------------------------------------------
            ...
            def salvar_man():
                ...
                try:
                    ...
                    self.db_conn.cursor().execute("INSERT INTO tb_transacoes (conta_id, categoria_id, valor, data_transacao, descricao) VALUES (?, ?, ?, ?, ?)", (mapa_contas[combo_conta.get()], cat, abs(v), d_iso, entry_desc.get()))
                    self.db_conn.commit()
                    self.rotear_tela("dash")
                except: pass
            ...
            def imp_csv():
                ...
                if cam and mapa_contas:
                    try:
                        ...
                        self.db_conn.cursor().executemany("INSERT INTO tb_transacoes (conta_id, categoria_id, valor, data_transacao, descricao) VALUES (?, ?, ?, ?, ?)", reg)
                        self.db_conn.commit(); self.rotear_tela("dash")
                    except: pass
        ...
        # -------------------------------------------------------------
        # ROTA: PLANEJAMENTO (MÁQUINA DE ESTADO ANINHADA)
        # -------------------------------------------------------------
            ...
            if aba == 'orcamentos':
                ...
                f_split.grid_columnconfigure((0,1), weight=1)
                ...
                ctk.CTkLabel(f_forjar, text="Forjar Teto (Mês Atual)", font=ctk.CTkFont(weight="bold")).pack(pady=20)
                
                try:
                    cursor = self.db_conn.cursor()
                    cursor.execute("SELECT id, nome FROM tb_categorias WHERE tipo_fluxo = 'DESPESA'")
                    mapa_cat = {nome: id_c for id_c, nome in cursor.fetchall()}
                    l_cat = list(mapa_cat.keys()) if mapa_cat else ["Erro"]
                except: l_cat = []; mapa_cat = {}

                cb_cat = ctk.CTkOptionMenu(f_forjar, values=l_cat, fg_color=COR_FUNDO, height=45, corner_radius=10)
                cb_cat.pack(pady=10, padx=20, fill="x")
                e_teto = ctk.CTkEntry(f_forjar, placeholder_text="Teto (R$)", height=45, corner_radius=10)
                e_teto.pack(pady=10, padx=20, fill="x")

                def gravar_teto():
                    if e_teto.get() and mapa_cat:
                        mes = datetime.now().strftime('%Y-%m')
                        c = self.db_conn.cursor()
                        c.execute("DELETE FROM tb_orcamentos WHERE categoria_id=? AND mes_referencia=?", (mapa_cat[cb_cat.get()], mes))
                        c.execute("INSERT INTO tb_orcamentos (categoria_id, teto_gasto, mes_referencia) VALUES (?, ?, ?)", (mapa_cat[cb_cat.get()], abs(float(e_teto.get())), mes))
                        self.db_conn.commit(); self.rotear_tela("planejamento")
                ctk.CTkButton(f_forjar, text="Travar Teto", fg_color=COR_PRIMARIA, height=45, corner_radius=10, command=gravar_teto).pack(pady=20, padx=20, fill="x")

                p_prog = ctk.CTkScrollableFrame(f_split, fg_color=COR_CARD, corner_radius=15, border_width=1, border_color=COR_BORDAS)
                p_prog.grid(row=0, column=1, sticky="nsew", padx=(10, 0))
                ctk.CTkLabel(p_prog, text="Burn Rate", font=ctk.CTkFont(weight="bold")).pack(pady=10)

                try:
                    mes = datetime.now().strftime('%Y-%m')
                    cursor.execute("SELECT c.nome, o.teto_gasto, COALESCE(SUM(t.valor), 0) FROM tb_orcamentos o JOIN tb_categorias c ON o.categoria_id = c.id LEFT JOIN tb_transacoes t ON t.categoria_id = c.id AND strftime('%Y-%m', t.data_transacao) = o.mes_referencia WHERE o.mes_referencia = ? GROUP BY c.nome, o.teto_gasto", (mes,))
                    for n, t, g in cursor.fetchall():
                        cd = ctk.CTkFrame(p_prog, fg_color=COR_FUNDO, corner_radius=8)
                        cd.pack(fill="x", pady=5, padx=10, ipadx=10, ipady=10)
                        pct = min(g/t, 1.0) if t>0 else 1.0
                        cor = COR_RECEITA if pct<0.7 else "#F1C40F" if pct<0.9 else COR_DESPESA
                        ctk.CTkLabel(cd, text=f"{n} ({pct*100:.1f}%)", text_color=cor, font=ctk.CTkFont(weight="bold")).pack(anchor="w")
                        # [ENG] Barra Progresso iOS HUD
                        br = ctk.CTkProgressBar(cd, progress_color=cor, fg_color="#2A2A3D", height=12)
                        br.pack(fill="x", pady=5); br.set(pct)
                        ctk.CTkLabel(cd, text=f"{self.aplicar_mascara_financeira(g)} de {self.aplicar_mascara_financeira(t)}", text_color=COR_TEXTO, font=ctk.CTkFont(size=11)).pack(anchor="w")
                except: pass
                ...
                f_meta = ctk.CTkFrame(f_split, fg_color=COR_CARD, corner_radius=15, border_width=1, border_color=COR_BORDAS)
                ...
                e_nm = ctk.CTkEntry(f_meta, placeholder_text="Nome do Objetivo", height=45, corner_radius=10)
                ...
                def g_meta():
                    if e_nm.get() and e_alv.get():
                        self.db_conn.cursor().execute("INSERT INTO tb_metas (nome, valor_alvo, valor_acumulado) VALUES (?, ?, ?)", (e_nm.get(), float(e_alv.get()), float(e_ap.get() or 0)))
                        self.db_conn.commit(); self.rotear_tela("planejamento", {'aba': 'metas'})
                ctk.CTkButton(f_meta, text="Gravar Meta", fg_color=COR_PRIMARIA, height=45, corner_radius=10, command=g_meta).pack(pady=20, padx=20, fill="x")

                p_track = ctk.CTkScrollableFrame(f_split, fg_color=COR_CARD, corner_radius=15, border_width=1, border_color=COR_BORDAS)
                ...
                try:
                    self.db_conn.cursor().execute("SELECT nome, valor_alvo, valor_acumulado FROM tb_metas")
                    for n, a, ac in self.db_conn.cursor().fetchall():
                        cd = ctk.CTkFrame(p_track, fg_color=COR_FUNDO, corner_radius=8)
                        cd.pack(fill="x", pady=5, padx=10, ipadx=10, ipady=10)
                        pct = min(ac/a, 1.0) if a>0 else 1.0
                        ctk.CTkLabel(cd, text=f"{n} ({pct*100:.1f}%)", text_color=COR_RECEITA, font=ctk.CTkFont(weight="bold")).pack(anchor="w")
                        
                        # [ENG] Barra de Metas (Cinza escuro = falta, Verde = Alocado)
                        br = ctk.CTkProgressBar(cd, progress_color=COR_RECEITA, fg_color="#2A2A3D", height=15)
                        br.pack(fill="x", pady=5); br.set(pct)
                        ctk.CTkLabel(cd, text=f"{self.aplicar_mascara_financeira(ac)} de {self.aplicar_mascara_financeira(a)}", text_color=COR_TEXTO, font=ctk.CTkFont(size=11)).pack(anchor="w")
                except: pass
